...
describe("Testing the echo endpoint", () => {
  ...
  test("testing echo endpoint when there's a file", async () => {
  ...
  test("test echo endpoint when there's no file", async () => {
    const response = await request(app).post("/echo").attach("file", null);
    expect(response.body.status).toBe(402);
    expect(response.body.message).toMatch("No document file uploaded");
    expect(response.type).toBe("application/json");
  });

  test("test echo endpoint when a wrong file format", async () => {
  ...
  test("test echo endpoint for invalid csv input", async () => {
    const response = await request(app)
      .post("/echo")
